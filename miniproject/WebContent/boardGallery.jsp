	...
	<div class="card-body">
		...
		<div class="row" style="width:100%;margin:auto">
			...
			<table class="table table-sm"
				...
				<tr>
					<td colspan="2"><a href="boardView.jsp?bid=<%=bid%>&num=<%=dto.getNum()%>"><img src="./storage/<%=dto.getAttatch()%>"
						style="width: 180px; height: 180px"></a></td>
				</tr>
				<tr>
					<td colspan="2"><a href="boardView.jsp?bid=<%=bid%>&num=<%=dto.getNum()%>"><%=dto.getTitle()%></a> <span
						class="badge badge-secondary" style="display:<%=(dto.getRcnt() > 0) ? "inline-block" : "none"%>"><%=dto.getRcnt()%></span> <span class="badge badge-warning text-light"
					style="display:<%=(timepassed<60)?"inline-block":"none"%>">NEW</span></td>
				</tr>
				<tr>
					<td><a href="memberinfo.jsp?uid=<%=mdto.getId()%>"><img
							src="./profiles/<%=mdto.getProfile()%>" class="rounded-circle"
							style="width: 25px; height: 25px; border: 1px solid #BDBDBD;"></a>
						<%=dto.getWname()%></td>
					<td style="text-align: right">조회 : <%=dto.getHit()%></td>
				</tr>

				<tr>
					<td>작성일</td>
					<td style="text-align: right"><%=(istoday) ? nodate.format(dt) : notime.format(dt)%></td>
				</tr>
			</table>
