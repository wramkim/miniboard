...
<script>
	function checkDone() {
		opener.document.getElementById("bid").value = "<%=request.getParameter("bid")%>";
		window.close();
	}
</script>
</head>
<body style="overflow: hidden;">
	<div class="card" style="width: 500px; height: 350px; margin: auto;">
		...
		<div class="card-body" style="text-align: center">
			...
			%>
			<div class="alert alert-danger">BID를 입력해주세요.</div>
			<%
				return;
				}
			...
			%>
			<div class="alert alert-danger">BID는 영어 소문자(a-z)와 숫자(0-9),
			...
			<%
				...
				if (ms.checkBID(bid)) {
			%>
			<div class="alert alert-success">사용 가능한 BID입니다.</div>
			<button class="btn btn-success" type="button" onclick="checkDone();">사용하기</button>
			<%
				} else {
			%>
			<div class="alert alert-danger">이미 사용중인 BID입니다.</div>
			<%
				}
			%>
		</div>
